...
const VEHICLES = {
  ...
  "Box Truck (16–26ft)": { sqft: [180, 350], icon: "📦", flat: true },
...
const COVERAGE = {
  ...
  "Spot Graphics / Lettering": { multiplier: 0.25, label: "Logo, phone, essentials — starting at $800" },
};

const MATERIALS = {
  "Premium Cast Vinyl (3M / Avery)": { _m: 1, desc: "Industry gold standard — vibrant, durable, 7+ year life" },
  "Standard Calendered Vinyl": { _m: 0.9, flatOnly: true, desc: "Made for flat surfaces — trailers, storefronts & box trucks" },
  "Reflective Vinyl": { _m: 2, desc: "Visible day & night — ideal for service vehicles" },
  "Chrome / Specialty Finish": { _m: 2, desc: "Head-turning metallic & specialty looks — 1 year warranty" },
};

// --- Internal pricing (not exposed to client) ---
const _RATE = 15;        // $/sqft
const _OVERPRINT = 1.15; // 15% overprint
const _DESIGN_FEE = 500; // flat design fee
const _MARKUP = 2.5;     // final output multiplier
...
export default function WrapCalculator() {
  ...
  const calculate = () => {
    ...
    let unitPrice;
    if (coverage === "Spot Graphics / Lettering") {
      // $800 base, scales up for larger vehicles
      unitPrice = Math.round(800 + Math.max(0, v.sqft[0] - 50) * 8);
    } else {
      unitPrice = Math.round((v.sqft[0] * c.multiplier * _OVERPRINT * _RATE * m._m + _DESIGN_FEE) * _MARKUP);
    }
            ...
            {Object.entries(VEHICLES).map(([name, data]) => (
              <OptionCard
                ...
                onClick={() => {
                  ...
                  if (!data.flat && material === "Standard Calendered Vinyl") setMaterial("");
                }}
                icon={data.icon}
                label={name}
                sub={null}
        ...
        >
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 10 }}>
            {Object.entries(MATERIALS)
              .filter(([, data]) => !data.flatOnly || (vehicle && VEHICLES[vehicle].flat))
              .map(([name, data]) => (
              <OptionBar
                key={name}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            <button
              ...
              style={{
                ...
                color: "#94a3b8", fontSize: 22, fontFamily: "inherit", cursor: "pointer",
            ...
            {qty > 1 && (
              <span style={{ fontSize: 13, color: "#3b82f6", fontWeight: 500, marginLeft: 4 }}>
          ...
          <div ref={resultRef} style={{
            ...
            <div style={{
              ...
              <div style={{ fontSize: 14, color: "#64748b", marginBottom: 28 }}>
                for {coverage.toLowerCase()} on {qty > 1 ? `${qty} vehicles` : vehicle.toLowerCase()}{qty > 1 ? ` (${vehicle.toLowerCase()})` : ""}
              </div>
