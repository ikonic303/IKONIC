...
const MATERIALS = {
  "Premium Cast Vinyl (3M / Avery)": { desc: "Industry gold standard — vibrant, durable, 7+ year life" },
  "Standard Calendered Vinyl": { desc: "Reliable quality — great for fleet & high-volume jobs" },
  "Reflective Vinyl": { desc: "Visible day & night — ideal for service vehicles" },
  "Chrome / Specialty Finish": { desc: "Head-turning metallic & specialty looks" },
};
...
export default function WrapCalculator() {
  ...
  const calculate = () => {
    ...
    const low = Math.round((v.sqft[0] * c.multiplier * _OVERPRINT * _RATE + _DESIGN_FEE) * _MARKUP);
    const high = Math.round((v.sqft[1] * c.multiplier * _OVERPRINT * _RATE + _DESIGN_FEE) * _MARKUP);
    return { low, high };
  };
